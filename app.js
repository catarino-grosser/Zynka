...
const topAdEl = document.getElementById('topAd');
...
function getAds(){
  return (window.ANUNCIOS_ZYNKA || ANUNCIOS_ZYNKA || []).filter(ad => ad && ad.ativo !== false);
}
...
function pickAd(index = 0){
  ...
  const saved = Number(localStorage.getItem('zynkaAdRotation') || '0');
  return ads[(saved + index) % ads.length];
}
...
function rotateAds(){
  ...
  const saved = Number(localStorage.getItem('zynkaAdRotation') || '0');
  localStorage.setItem('zynkaAdRotation', String((saved + 1) % ads.length));
}

function adTemplate(ad, extraClass = ''){
  if(!ad) return '';
  const img = ad.imagem || PLACEHOLDER_IMAGE;
  const link = ad.link || '#';
  return `
    <article class="ad-card ${extraClass}">
      <img class="ad-image" src="${escapeHTML(img)}" alt="Publicidade: ${escapeHTML(ad.titulo)}" loading="lazy" onerror="this.onerror=null;this.src='${PLACEHOLDER_IMAGE}'">
      <div class="ad-body">
        <span class="ad-label">Publicidade</span>
        <h3>${escapeHTML(ad.titulo)}</h3>
        <p>${escapeHTML(ad.descricao || '')}</p>
        <a class="ad-link" href="${escapeHTML(link)}" target="_blank" rel="noopener noreferrer">${escapeHTML(ad.botao || 'Saiba mais')}</a>
      </div>
    </article>`;
}

function renderAdsAreas(){
  const ads = getAds();
  if(!ads.length){
    topAdEl.innerHTML = '';
  ...
  topAdEl.innerHTML = adTemplate(pickAd(0), 'ad-top');

  const partners = [pickAd(1), pickAd(2), pickAd(3)].filter(Boolean);
  partnersEl.innerHTML = `
    <div class="section-title">
      <h2>Parceiros do Zynka</h2>
      <p>Projetos, negócios e indicações</p>
    </div>
    <div class="partners-grid">
      ${partners.map(ad => adTemplate(ad, 'ad-partner')).join('')}
    </div>`;
}
...
function renderGridWithAds(items){
  const html = [];
  items.forEach((item, index) => {
    html.push(cardTemplate(item));
    if((index + 1) % 6 === 0){
      html.push(adTemplate(pickAd(index + 4), 'ad-inline'));
    }
  });
  return html.join('');
}
...
function render(items){
  ...
  gridEl.innerHTML = renderGridWithAds(rest);
